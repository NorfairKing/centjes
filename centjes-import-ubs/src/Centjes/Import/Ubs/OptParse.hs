...
module Centjes.Import.Ubs.OptParse where
...
combineToSettings Flags {..} Environment {..} mConf = do
  ...
  settingInput <- resolveFile' $ fromMaybe "ubs.cent" $ flagInput <|> envInput
  settingOutput <- resolveFile' $ fromMaybe "ubs.cent" $ flagOutput <|> envOutput
  let settingAssetsAccountName = fromMaybe (AccountName "assets:ubs") $ flagAssetsAccountName <|> envAssetsAccountName <|> mc configAssetsAccountName
  let settingExpensesAccountName = fromMaybe (AccountName "expenses:unknown:ubs") $ flagExpensesAccountName <|> envExpensesAccountName <|> mc configExpensesAccountName
  let settingIncomeAccountName = fromMaybe (AccountName "income:unknown:ubs") $ flagIncomeAccountName <|> envIncomeAccountName <|> mc configIncomeAccountName
  let settingFeesAccountName = fromMaybe (AccountName "expenses:banking:ubs") $ flagFeesAccountName <|> envFeesAccountName <|> mc configFeesAccountName
  pure Settings {..}
  where
...
getConfiguration Flags {..} Environment {..} =
  case flagConfigFile <|> envConfigFile of
    Nothing -> resolveFile' "ubs.yaml" >>= readYamlConfigFile
    Just cf -> do
      afp <- resolveFile' cf
